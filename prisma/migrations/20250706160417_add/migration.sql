/*
  Warnings:

  - A unique constraint covering the columns `[slug]` on the table `Negocio` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `slug` to the `Negocio` table without a default value. This is not possible if the table is not empty.

*/
-- AlterTable
ALTER TABLE "Negocio" ADD COLUMN     "slug" TEXT NOT NULL;

-- CreateIndex
CREATE UNIQUE INDEX "Negocio_slug_key" ON "Negocio"("slug");
