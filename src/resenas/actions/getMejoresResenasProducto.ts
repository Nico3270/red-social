...
interface GetMejoresResenasProductoResult {
  ok: boolean;
  message: string;
  resenas?: ResenaProducto[];
}

export const getMejoresResenasProducto = async (productSlug: string, take = 10): Promise<GetMejoresResenasProductoResult> => {
  ...
  try {
    ...
    const publicaciones = await prisma.publicacion.findMany({
      ...
      orderBy: [
        { calificacion: "desc" }, // Primero las de mayor calificación
        { createdAt: "desc" }, // Secundario: más recientes
      ],
      take, // Limitar el número de resultados
    });
    ...
    return {
      ok: true,
      message: "Mejores reseñas obtenidas exitosamente",
      resenas,
    };
  } catch (error) {
    console.error("Error en getMejoresResenasProducto:", error);
    return {
      ok: false,
      message: `Error al obtener las mejores reseñas: ${error instanceof Error ? error.message : "Error desconocido"}`,
    };
  }
};