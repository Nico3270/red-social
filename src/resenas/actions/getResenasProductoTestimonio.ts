...
interface GetResenasProductoTestimonioResult {
  ok: boolean;
  message: string;
  resenas?: ResenaProducto[];
}

export const getResenasProductoTestimonio = async (productSlug: string): Promise<GetResenasProductoTestimonioResult> => {
  // Validar slug
  if (!productSlug || typeof productSlug !== "string" || productSlug.trim() === "") {
    return { ok: false, message: "Slug del producto inválido o requerido", resenas: [] };
  }

  // Obtener usuario autenticado (para userReaction)
  ...
  try {
    ...
    if (!producto) {
      return { ok: false, message: "Producto no encontrado", resenas: [] };
    }
    ...
    // Formatear reseñas para EnhancedPublicacion
    ...
    console.error("Error en getResenasProductoTestimonio:", error);
    return {
      ok: false,
      message: `Error al obtener las reseñas: ${error instanceof Error ? error.message : "Error desconocido"}`,
      resenas: [],
    };
  }
};
