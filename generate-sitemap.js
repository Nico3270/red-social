...
} = require("./src/lib/business/business-visibility-policy.ts");

const prisma = new PrismaClient();
const siteUrl = process.env.SITE_URL || "https://myckeo.com";

async function fetchDynamicEntities(client = prisma) {
  const [negocios, productos, categorias, encuestas] = await Promise.all([
    ...
    client.category.findMany({ select: { slug: true, updatedAt: true } }),
    client.encuesta.findMany({
      where: { negocio: buildPublishedBusinessRelationWhere() },
...
function buildSitemapContent({ negocios, productos, categorias, encuestas }) {
...
module.exports = { buildSitemapContent, fetchDynamicEntities, main };
